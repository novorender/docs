    ...
    try {
        ...
        canvas.onclick = async (e) => {
            if (currentOutput) {
                ...
                if (result1) {
                    ...
                    console.log('draw 123 ', draw);

                    await draw2d(_measureApi, view, context2D, canvas2D, glMatrix, draw as MeasureAPI.DrawProduct);
                }
            }
        };
        ...
        previewCanvas.onclick = (e) => {
            if (previewCanvas && preview && previewCanvasContext2D) {
                ...
                // previewCanvasContext2D.lineWidth = 2;
                // previewCanvasContext2D.strokeStyle = "black";
                // previewCanvasContext2D.beginPath();
                // previewCanvasContext2D.arc(x, y, 5, 0, 2 * Math.PI);
                // previewCanvasContext2D.fill();
                // previewCanvasContext2D.stroke();
                ...
                if (preview && previewCanvasContext2D) {
                    ...
                    img.onload = function () {
                        if (previewCanvasContext2D && preview) {
                            ...
                            // previewCanvasContext2D.drawImage(img, 450, 200, previewCanvas.naturalWidth * 0.7, previewCanvas.naturalHeight * 0.7, 0, 0, width, height);
                            previewCanvasContext2D.drawImage(img, 0, 0, previewCanvas.width, previewCanvas.height);
                            imgHeight = img.height;
                            // imgWidth.current = img.width;
                            // const w = previewCanvas.width / img.width;
                            // const h = previewCanvas.height / img.height;
                ...
                if (pdfPosA && pdfPosB && draw) {
                    ...
                    const modelPosA = fromValues(posA[0] / 5, posA[2] * -1 / 5);
                    const modelPosB = fromValues(posB[0] / 5, posB[2] * -1 / 5);
                    //Invert Y axis on the pixel positions on the pdf image
                    const pixelPosA = fromValues(pdfPosA[0] / 5, imgHeight - pdfPosA[1]);
                    const pixelPosB = fromValues(pdfPosB[0] / 5, imgHeight - pdfPosB[1]);
                    ...
                    const angleAroundZ = dot(modelDir, pixDir);
...
export function drawProduct(
    ...
    // const colorSettings: Array<ColorSettings> = [{ fillColor: 'red', outlineColor: 'red', lineColor: 'red' }, { fillColor: 'green', outlineColor: 'green', lineColor: 'green' }];
    for (const obj of product.objects) {
        ...
        obj.parts.forEach((part, i) => {
            ...
            drawPart(ctx, camera, part, pixelWidth, glMatrix);
        });
        // }
    }
}
...
function calculate() {
    console.log('total 4 points selected ');
}
