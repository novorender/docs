  ...
  "rules": [
    {
      "rule": [
        {
          "csv": "Test-csv.csv",
          "property": "Email", // CSV key
          "key": "Test" // Novorender property name
        }
      ]
    }
  ]
...
:::info
The optional payload property `key` is used to specify which property in Novorender should be linked to a property (`property`) from the CSV file. If left empty or undefined, it is assumed that the Novorender property name matches the CSV property name (`property`).
:::
