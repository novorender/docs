...
:::caution
Overestimating your GPU capabilities will cause slow frame rates and can cause the browser to terminate the GPU context, resulting in flickering and page reloads.
...
If so, you can pass and empty object `{}` as an argument.
...
As long as you can determine when and what parts of that viewstate has changed, e.g. by using immutable data and reference comparisons [like we do](render_state),
