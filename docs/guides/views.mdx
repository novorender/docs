...
The view expects that you provided it with all imports through a <CodeLink type="type" name="ViewImports"/> object.
...
<PlaygroundComponent {...snippets.views.creation} />
...
<PlaygroundComponent {...snippets.views.animation} />

## Environments

We used image based lighting ([IBL](https://en.wikipedia.org/wiki/Image-based_lighting)) to light and shade the geometry.
Most of our examples use the default environment, which is a boring 6-pixel cube map with shades of gray.
Such a simple environment doesn't do the engine any justice and is just there so that at least there is some light by default.

To use a proper environment map, we need to download a set of HDRI cube texture maps with pre-convoluted irradiance.
The process of how to generate those from an original HDR panorama image is outside the scope of this guide.
Instead we'll show you how to pick and download one of the default environments.
...
<PlaygroundComponent {...snippets.views.environments} />
...
:::tip
If you don't want the skybox background to stand out, you can blur it using <CodeLink type="interface" name="RenderStateBackground.blur"/>.
:::
