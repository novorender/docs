...
Although the [ObjectGroup interface](https://docs.novorender.com/docs/data-js-api/interfaces/DataJsAPI.ObjectGroup) itself is not particularly noteworthy,
the <code>[SceneData.objectGroups](https://docs.novorender.com/docs/data-js-api/interfaces/DataJsAPI.SceneData#objectgroups)</code> property can be extremely useful.
...
When the <code>[objectGroups.ids](https://docs.novorender.com/docs/data-js-api/interfaces/DataJsAPI.Bookmark.ObjectGroup#ids)</code> list becomes extensive, the `.ids` property will be `undefined` and must be explicitly loaded, as demonstrated in the [Floors demo](#floors-example).
...
In this example, we've established predefined groups for each floor and implemented buttons to isolate [objectGroups](https://docs.novorender.com/docs/data-js-api/interfaces/DataJsAPI.Bookmark-1#objectgroups) within the group corresponding to the selected floor.
