...
<PlaygroundComponent demoName={tutorialDemo.demoName} config={{useManagedRenderer: false, clickToRun: true }} editUrl={'demo-snippets/tutorials/dynamic_objects.snippet.ts'}>{tutorialDemo.main}</PlaygroundComponent>
...
> Note that when disposing an asset, the render pipeline may still have a reference to the model for the next frame.
