...
export const CodeExamplesTip = () => (
  <div
    ...
    <div style={{display: 'flex', justifyContent: 'center'}}>{['vite', 'webpack', 'local_esm'].map(i => 
      <Link to={`https://github.com/novorender/novorender-examples/tree/master/${i}`} key={i} style={{margin: 5}}><img src={require(`../../static/img/${i}.png`).default} style={{width: 100, borderRadius: 5}} /></Link>
    )}</div>
  </div>
);
...
export const PlaygroundTip = () => (
  <div
    ...
    <div style={{display: 'flex'}}>
      ...
      <Link to="/playground" className="button button--primary" style={{display: 'block', margin: '0 auto', maxWidth: 300}}>Go to Playground</Link>
  </div>
);
