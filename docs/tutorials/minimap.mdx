...
Next, we'll create a function that will be responsible for creating the view and the camera controller for the 3D view:
```typescript
...
```
this function will be called after loading the scene.
...
We start by creating a rectangle to retrieve the quads that intersect with it:
```typescript
 const area: NodeGeometry = {
  x: Math.max(0, centerX - width / 2),
  y: Math.max(0, centerY - height / 2),
...
export class MinimapHelper {
   ...
   */
  constructor(width: number, height: number, readonly minimaps: MinimapInfo[]) {
