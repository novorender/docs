...
In this introductory tutorial, we will explore the process of configuring the Novorender Web API by developing a simple web application.

Our chosen module bundler for this tutorial will be [Vite](https://vitejs.dev). We've selected Vite because of its speed, straightforward setup, and excellent support for ESM (ECMAScript Modules). However, please note that you are free to use other bundlers like webpack, parcel, or rollup if you prefer.

## Setup

To begin, let's initiate a new TypeScript project from scratch:
...
<details>
  <summary>Enabling Cross Origin headers in Webpack</summary>

Webpack users can also enable cross origin headers by adding the following code snippet to their `webpack.config.js` file
...
*Todo: Basic scene loading tutorial will go here when data-js-api gets integrated into new web api.*

## Measure

*Todo: add basic measure functionality after scene loading.*
