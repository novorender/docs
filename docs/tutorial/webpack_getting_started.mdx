...
```
:::info
Did you notice the `// @ts-expect-error` comment? This is essential because TypeScript might incorrectly assume that the `@novorender/api` doesn't expose this module, even though it does and this will tell TypeScript to suppress that error from being reported.
:::
