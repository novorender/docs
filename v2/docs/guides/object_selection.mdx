...
- <code>
    <b>"hide"</b>
  </code> will hide the objects within the group more effectively than using alpha/opacity = 0, while still retaining them in memory for quick re-rendering.
- <code>
    <b>"filter"</b>
  </code> will remove the objects from memory entirely, which helps improve rendering performance and memory usage. Unlike hidden group, changes to filtered groups will trigger a complete reload of the streamable scene.
