...
[three.js](https://threejs.org/) is a good example of how this is far from obvious.
...
Yes, copying is slower than mutation, but that cost is easily recovered by the efficieny of basic reference comparisons to detect changes.
Not to mention the simplicity of understanding, use, implementation and a host of other benefits.

## How to modify render state.

Now that we've hammered down the why, let's see how render state should be changed.
...
To simplicity matters and help encourage correct update of render state, we made the <CodeLink type="class" name="View.modifyRenderState"/> function.

```typescript
view.modifyRenderState({ background: { color: [0, 0, 0.1, 1] } });
```

This function will recursively traverse the `changes` argument, which is a _partial_ render state object, and apply to the new render state.
...
Due to the way changes trickle up to the parent object, `hasBackgroundChanged` will be true of any of the background state has changed, including color.
...
It only perform basic range checks and values that could result in a run-time exception, such as making sure expected integers are indeed integers.

Some examples will do validation and report back validation errors:

<PlaygroundComponent {...snippets.gettingStarted["Render state validation"]} />
