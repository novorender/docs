...
To help your team avoid accidental in-place mutation, you should use the typescript `readonly` keyword and `as const` whenever possible, preferably on both sides of array, like illustrated below.
...
We strive to do this everywhere, except where the intention is explictly that of mutation, which is rare.
...
This style of coding may feel awkward and alien to you at first.
Anyone who has used the popular [`tree.js`](https://threejs.org/) rendering engine will appreciate the consistency of immutable state and the absence of non-obvious `update()` methods.
Yes, mutation is generally faster - when done right.
But so is coding in machine code.
