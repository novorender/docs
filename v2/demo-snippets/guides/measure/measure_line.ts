import { DrawPart, DrawProduct, OrbitController, View } from "@novorender/api";
  ...
  view.canvas.onclick = async (e: MouseEvent) => {
    ...
    if (result) {
      ...
      if (context2D && point1 && point2) {
        ...
        const { rotation, position } = view.activeController as OrbitController;
        ...
        if (drawProd) {
          ...
          drawProduct(context2D, camSettings, drawProd, { lineColor: "green" }, 3);
        }
      }
    }
  };
...
export function drawProduct(ctx: CanvasRenderingContext2D, camera: CameraSettings, product: DrawProduct, colorSettings: ColorSettings, pixelWidth: number, textSettings?: TextSettings, lineCap?: LineCap) {
...
export function drawPart(ctx: CanvasRenderingContext2D, camera: CameraSettings, part: DrawPart, colorSettings: ColorSettings, pixelWidth: number, textSettings?: TextSettings, lineCap?: LineCap): boolean {
  if (part.vertices2D) {
