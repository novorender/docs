...
export default function Playground(): JSX.Element {
    ...
    useEffect(() => {
        ...
        const tutsList = [...Object.keys(tutorials).map(k => {
                        ...
                        <nav className="navbar fade-in" style={{ borderRadius: 10 }}>
                            <div className="navbar__inner">
                                <div className="navbar__items">
                                    <div className="navbar__brand" style={{ fontWeight: 'bold' }}>{demo_grp.dirName}</div>
                                </div>
                                {/* <div className="navbar__items navbar__items--right"></div> */}
                            </div>
                        </nav>

                        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', padding: '1rem 0' }}>{demo_grp.demos.map((t: IDempProps, i: number) => <div key={t.demoName} style={{ margin: 5, animationDuration: i * 250 + 'ms' }} className="fade-in-top">{renderCard(demo_grp.dirName, t)}</div>)}</div>
                    </div>
                )
                }
            </div>
